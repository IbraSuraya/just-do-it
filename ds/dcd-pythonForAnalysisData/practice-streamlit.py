...
# Tujuan
# - membuat sebuah report analisis data yang interaktif dalam sebuah dashboard
...
# change type data to datetime
datetime_columns = ["order_date", "delivery_date"]
for column in datetime_columns:
  all_df[column] = pd.to_datetime(all_df[column])

# setting default date min and max
min_date = all_df["order_date"].min()
max_date = all_df["order_date"].max()

# date input sebagai filternya di sidebar
with st.sidebar:
...
# informasi terkait daily orders : jumlah order harian serta total order dan revenue dalam range waktu tertentu. 
...
# performa penjualan dari setiap produk dengan 5 produk paling laris dan paling sedikit terjual 
...
# nilai average atau rata-rata dari ketiga parameter
...
st.pyplot(fig)
st.caption('Copyright (c) Dicoding 2023 x Ibra')